...
exports.signup = catchAsync(async (req, res, next) => {
  const newUser = await User.create({
    // limit registration input options
    name: req.body.name,
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (!user || !(await user.correctPassword(password, user.password))) {
    // If not, throw an error with status code 401
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(
      new AppError('you are not logged in! pleace log in to get access', 401),
    );
  }
...
exports.restrictTo =
  ...
  (req, res, next) => {
    if (!roles.includes(req.user.role)) {
      return next(
        new AppError('you do not gave permission to perform this action', 403),
      );
    }
    next();
  };
